...
Each skill lives under `skills/` as its own directory containing a `SKILL.md` and (optionally) a `references/` folder of supporting guides. There are currently five skills, designed to run back-to-back as a greenfield requirements-to-running-skeleton pipeline (`requirements-engineering` → `software-architecture` → `ux-foundations` → `implementation-planning` → `project-scaffolding`; further steps like detailed-design, ui-design, testing, and build are referenced as downstream but not yet built):
...
- **The RTM (`docs/rtm.md`) is a shared multi-writer ledger with exclusive column ownership** — the pipeline's spine of traceability. `requirements-engineering` owns the rows + all requirement columns and initializes Design/Plan/Test refs as `_TBD_`; `software-architecture` fills **Design ref** (ADR refs, per a layer rule), `implementation-planning` fills **Plan ref** (the scheduling slice), a future testing phase fills **Test ref**; `project-scaffolding` writes nothing. Every downstream writer **appends, never overwrites**, writes only its own column at its own delivery time, and **skips silently when there's no RTM** (standalone runs). See `requirements-engineering/references/rtm-guide.md` for the contract — respect it when editing any downstream skill's delivery phase.

So changes to what an upstream skill emits (requirement IDs/vocabulary, SCR IDs, surface naming, container vocabulary, the screen-inventory shape, RTM column ownership) can ripple downstream into how the next skill ingests it. Keep the vocabulary consistent across the five when editing any one.
