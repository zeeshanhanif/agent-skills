...
- `skills/software-architecture/` — **consumes the SRS** (defaults to reading `docs/srs.md`) and produces the architecture document (`docs/architecture.md`). Designs against the SRS's requirements/NFRs and interviews only for the gaps; falls back gracefully to a full interview when no SRS exists, so it still works standalone.
...
- `software-architecture` now **ingests `docs/srs.md`**: it reads the requirements and NFRs, designs against them, records which SRS requirement IDs drive each decision, and cites those IDs in its ADRs (the "Requirements addressed" field) — so traceability runs requirement → decision. This closes the earlier gap **for this skill only**: `ux-foundations` and `implementation-planning` still do *not* read `srs.md`/`use-cases.md` — they default to their own upstream design docs (`architecture.md`, `ux-foundations.md`). Wiring those two to also consume the requirements/use cases remains an open follow-up; keep it in mind when editing them.
...
**It consumes the SRS (Input section + Phase 1).** Defaults to reading `docs/srs.md`; when present, the SRS is the primary source — Phase 1 is *ingest → play back the drivers (top NFRs by ID, constraints, capability areas) → interview only the gaps* (tech/stack preferences, lock-in tolerance, team skills, access patterns, deployment target), not a full re-interview. When no SRS exists it falls back to the full seven-round elicitation and stays fully usable standalone. The boundary: the SRS states *what* (requirements); architecture decides *how* (stack, stores, deployment) — read only *mandated* tech from the SRS constraints. Preserve this graceful fallback if you touch the phases.

**Requirement → decision traceability is a load-bearing feature.** Each Phase-2 decision records the SRS requirement IDs that drive it; each ADR cites them in its **"Requirements addressed"** field; and the document template's Introduction, Architecture Decisions, and Quality Requirements sections cite SRS NFR IDs (and §1 summarizes/links `docs/srs.md` rather than restating it). This back-link is what lets a later requirements amendment find the architecture it affects — keep it intact when editing the templates.
...
- `document-template.md` — arc42-based output, with sections tagged `[essential]` vs `[scale]` for right-sizing (Phase 3); cites SRS NFR IDs and links `docs/srs.md` rather than restating requirements.
...
- `adr-template.md` — one ADR per significant (costly-to-reverse) decision (Phase 4); each carries a **"Requirements addressed"** field citing the SRS IDs it satisfies.
