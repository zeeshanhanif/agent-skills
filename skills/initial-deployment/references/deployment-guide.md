...
deploy-notes), the topology (which services run where, what fronts them),
...
**The deploy-plan playback** is one tight block: environments to create,
...
verified command forms in deploy-notes as they're used — they become the
project's operational truth. Never recite remembered commands; never let a
note (even deploy-notes from a previous run) override what live docs and the
...
- Verify the wiring with a **non-secret canary** first (a dummy key the user
  ...
  never *what* they are.
- Nothing secret ever lands in: files this skill writes, deploy-notes, chat,
...
deployment findings. Update the two standing records: deploy-notes (the
...
Maintain `docs/.deploy-progress.md` — per environment, per unit, statuses
